...
class VonageVoiceModule(reactContext: ReactApplicationContext) :
    ...
    fun getAvailableAudioDevices(promise: Promise) {
        ...
        val devices =
            audioManager.getDevices(AudioManager.GET_DEVICES_ALL).map { device ->
        ...
        promise.resolve(
            Arguments.createArray().apply { devices.forEach { deviceMap -> pushMap(deviceMap) } }
        )
    }
        ...
        scope.launch {
            callController.calls.collect { call: Call ->
                EventEmitter.getInstance()
                    .sendEvent(Event.CALL_EVENTS, EventPayload.createCallEventPayload(call))
            }
        }
    }
